...
public class CodeGen {
	 ...
	 *
	 * @param tree
	 *            AST which forms the program.
	 */
	public static void writeProgram(PrintStream outputStream, Tree<Token> tree) {

		// Code needs to know the output stream for all small 'emit' methods
		// below.
		...
		Templater tr = new Templater(outputStream) {
			...
			public void toString(String s) {
				...
				case "CODE": // we need an inner scope since code is generated
								// as body of main()
					beginScope();
					// invent a mythical first arg
					newLocal("ARGS TO MAIN", "[~"); // array of string (~ will
		...
		// this is the 'boiler plate' code into which generated code is
		// inserted.
		// it is done this way to make it easier to change - and because
		// Templater already existed!
		String[] theText = new String[] { ".class public (~CLASSNAME~)\n\n", ".super java/lang/Object\n",
	...
	static void writeStatementCode(Tree<Token> tree) {
		...
		case IF: {
			...
			for (int i = 0; i < pairs; i += 2) {
				...
				if (test != null) { // not 'else' part
					Label nextTest = newLabel("NEXT TEST"); // for jump to next
															// elif/else
		...
		case WHILE: { // also do/end
			...
			if (testExpr != null) { // 'null' for do/end
				writeExpressionCode(tree.child(0), INT_TYPE); // expression to
																// test
	...
	public static boolean writeExpressionCode(Tree<Token> tree) {
		...
		if (kids == 0) {
			...
			return (token == NUMBER) ? INT_TYPE : (token == STRING) ? STR_TYPE : isStringVar(tree);
		}
		...
		case NE: { // first deal with different types
			 ...
			 * where 0 (i.e.false) is loaded onto the stack. There is then a
			 * jump to the end. The next line is the target when the test was
			 * true, -1 (true) is loaded onto the stack
			 */
